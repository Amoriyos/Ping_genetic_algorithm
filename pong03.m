...
MAX_BALL_SPEED = 4; %wont accelerate bast this, dont set too high or bugs.
BALL_ACCELERATION = 0.3; %how much ball accelerates each bounce.
PADDLE_SPEED = 2;
...
%called from main loop on every frame
    ...
    e=(sum(pa1(4:2:6))/2-s)^2; 
    end

    function e=movePaddles2(x1,f1)
    %set new paddle y locations
    persistent pal2
    if f1 ==0
        pa2=paddle2;
    ...
    pa2(2,:) = pa2(2,:) + (x1 );
    e=(sum(pa2(4:2:6))/2-ballY)^2;
    end

  function movePaddles(p1,p2)
    %set new paddle y locations
    paddle1(2,:) = paddle1(2,:) + (p1 );
...
%%
while ~quitGame
%     x=1*(sum(paddle1(4:2:6))/2-ballY)
%  pV = min(max(x,-2),2)
tic
  moveBall;
  func1= @(x) movePaddles1(x,0); 
  if 1 % choose 1 to genetic algorithm or 0 to run convex optimiztion
  ga_opts = gaoptimset('TimeLimit',0.001,'FitnessLimit', 5,'display','off');
  [x,fval,exitflag]=ga(func1,1,[],[],[],[],-1,1,[],ga_opts);
  else
    opts = optimoptions(@fmincon,'Algorithm','sqp','OptimalityTolerance',1e-2,'display','off');
 [x,fval,exitflag]=fmincon(func1,0,[],[],[],[],-1,1,[],opts);
  end
  movePaddles(x,x1);
  refreshPlot;
  checkGoal;
  toc
  %     func2= @(x1) movePaddles2(x1,0);
%    [x1,fval,exitflag]=fmincon(func2,0,[],[],[],[],-1,1,[],opts);
%   pause(abs(0.01-dt))
end
close(fig);
